  ...
  return (
    ...
    >
      ...
      {menuOpen && (
        ...
        >
          {["PDV", "Produtos", "Caixa"].map((item) => (
            <Link
              key={item}
              href={`/gigante/${item.toLowerCase()}`}
      ...
      <div style={{ maxWidth: 800, margin: "0 auto 40px" }}>
        ...
        >
          {[
            { img: "/promo1.jpeg", label: "Frango Assado R$ 49,90" },
            { img: "/promo2.jpeg", label: "Combo Família R$ 79,90" },
            { img: "/promo3.jpeg", label: "Costela no Bafo R$ 49,90" },
          ].map((promo, i) => (
            <SwiperSlide key={i}>
              <img
                src={promo.img}
              ...
              />
              <p style={{ marginTop: 10, fontWeight: "bold" }}>{promo.label}</p>
            </SwiperSlide>
          ))}
        </Swiper>
      </div>
